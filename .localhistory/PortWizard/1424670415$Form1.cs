    ...
    public partial class Form1 : Form
    {
        ...
        private void init()
        {
            ...
            if (ProcessListView.Items.Count == 0){
                ProcessListView.Items.Add("No processes using ports 80 or 443 found. If CM still not launching, try right click here and refresh to see if any new process running or restart app");
        ...
        private void ProcessListView_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                {
                    contextMenuStrip2.Show(listView, e.Location);
                }
            }
        }
        ...
        private void endProcessToolStripMenuItem_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem item in ProcessListView.SelectedItems)
            {
                try
                {
                    string selectedProcessID = item.SubItems[2].Text;
                    KillProcess(Convert.ToInt32(selectedProcessID));
                    MessageBox.Show("Ended " + item.Text);
                }
                catch (Exception ex1)
                {
                    MessageBox.Show("Problem ending " + item.Text + " \\ PID : " + item.SubItems[2] + " : " + ex1.ToString());
                }
            }
